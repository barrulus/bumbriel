...
vec4 postprocess(vec3 c){
    ...
    float edge  = 0.10 + hn * 0.56 - barrulus_smoothstep(0.04, 0.0, h) * 0.08;
    float core  = barrulus_smoothstep(edge, edge + 0.22, n);
    float halo  = barrulus_smoothstep(edge - 0.20, edge + 0.34, n) * (1.0 - hn * 0.5);

    float temp = core * (1.0 - barrulus_smoothstep(0.0, 1.0, hn) * 0.62);
    ...
    col        = mix(col, vec3(1.0, 0.94, 0.72),  barrulus_smoothstep(0.78, 1.0,  temp));

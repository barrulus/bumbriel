...
float cracks(vec2 p, float width) {
    ...
    return 1.0 - barrulus_smoothstep(0.0, width, edge);
}
...
vec4 postprocess(vec3 c) {
    ...
    float burn = barrulus_smoothstep(burnWidth, 1.05, edge);
    ...
    float paperMix = (1.0 - barrulus_smoothstep(0.06, inkKeep, lum)) * paperOpacity;

...
vec3 heat_color(float temp){
    vec3 col = mix(vec3(0.62, 0.05, 0.01), vec3(1.0, 0.34, 0.02), barrulus_smoothstep(0.02, 0.40, temp));
    col      = mix(col, vec3(1.0, 0.74, 0.12), barrulus_smoothstep(0.38, 0.70, temp));
    return     mix(col, vec3(1.0, 0.94, 0.72), barrulus_smoothstep(0.78, 1.00, temp));
}
...
vec4 postprocess(vec3 c){
    ...
    float thr = THRESH + pow(max(hn, 0.0), 0.75) * SLOPE
              - barrulus_smoothstep(0.05, 0.0, h) * 0.12;
    float fire = barrulus_smoothstep(thr - EDGE, thr + EDGE, n);
    float halo = barrulus_smoothstep(thr - 0.16, thr + 0.02, n);

    if (fire + halo <= 0.0) return s;

    float d    = clamp((n - thr) / 0.22, 0.0, 1.0);
    float temp = (0.55 + 0.45 * d) * (1.0 - barrulus_smoothstep(0.0, 1.25, hn) * 0.85);

    float mask = barrulus_smoothstep(0.0, 0.25, s.a);

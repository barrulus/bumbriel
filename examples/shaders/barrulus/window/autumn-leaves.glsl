...
vec4 postprocess(vec3 c){
    ...
    for (int i = 0; i < LEAVES; i++){
        ...
        float m      = barrulus_smoothstep(1.0, 0.80, d) * gate;

        vec3 lc = mix(vec3(0.72, 0.30, 0.10), vec3(0.87, 0.60, 0.16), barrulus_smoothstep(0.25, 0.60, r3));
        lc      = mix(lc, vec3(0.46, 0.44, 0.14), barrulus_smoothstep(0.70, 0.90, r1));
        lc     *= (0.80 + 0.30 * r2) * (0.62 + 0.38 * squash);
        float rib = barrulus_smoothstep(0.14, 0.03, abs(q.y) / max(sz * squash, 0.0001));

...
vec4 postprocess(vec3 c){
    ...
    for (int i = 0; i < 3; i++){
        ...
        float soft  = barrulus_smoothstep(colW * 0.20, 0.0, abs(px - xc));
        float v     = fract((a.y - t * vel) / 0.6 + rn * 13.0);
        float lenN  = 0.22 + 0.25 * rn;
        float seg   = barrulus_smoothstep(0.0, lenN, v) * (1.0 - barrulus_smoothstep(lenN, lenN + 0.10, v));
    ...
    for (int i = 0; i < 2; i++){
        ...
        float sz   = barrulus_smoothstep(0.0, 0.35, life) * barrulus_smoothstep(1.0, 0.8, life);
        float rad  = (0.10 + 0.22 * rn) * sz * on;
        vec2  d2   = f - p;
        float m    = barrulus_smoothstep(rad, rad * 0.55, length(d2)) * step(0.001, rad);

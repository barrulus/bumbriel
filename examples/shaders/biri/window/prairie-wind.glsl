...
vec3 pal(float x){
    vec3 col = mix(vec3(0.28, 0.38, 0.20), vec3(0.78, 0.64, 0.28), biri_smoothstep(0.15, 0.55, x));
    return mix(col, vec3(0.93, 0.88, 0.66), biri_smoothstep(0.60, 0.92, x));
}
...
vec4 postprocess(vec3 c){
    ...
    float gust  = biri_smoothstep(0.32, 0.70, g) * surge;
    float paw   = fbm(a * 2.4 * SCALE + vec2(-t * SWEEP * 1.2, t * 0.10));
    float burst = biri_smoothstep(0.50, 0.76, paw) * (0.55 + 0.45 * sin(t * 2.8 + paw * 12.0));
    ...
    float amt = biri_smoothstep(0.10, 0.65, gust) * STRENGTH * gate;

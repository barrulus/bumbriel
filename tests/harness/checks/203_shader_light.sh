...
passes = [{shader = "emitter.glsl"}]
[effects.fixture.border.outer.light]
enabled = true
spread = 40.0
intensity = 4.0
threshold = 0.5
...
"$UMBRIEL" effects --json | jq -e '.windows[0].scopes["border.outer"].source.effect == "fixture"' >/dev/null
grim "$IMAGE"
read -r outer_red outer_blue < <(sample "$((left - 12))")
(( outer_red > 10 && outer_blue < 5 )) || { echo "failed edit discarded working illumination"; exit 1; }
echo "post-opacity emission threshold, inward/outward scene lighting, and atomic rejection verified"

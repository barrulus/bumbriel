...
vec4 global_color(vec3 c){
    ...
    float ar = niri_size.x / max(niri_size.y, 1.0);    // keep flames the same width everywhere
    float t  = niri_time;
    ...
    float edge  = 0.10 + hn * 0.56 - smoothstep(0.04, 0.0, h) * 0.08;   // softly anchored at the base
    float core  = smoothstep(edge, edge + 0.22, n);    // solid flame body
    float halo  = smoothstep(edge - 0.20, edge + 0.34, n) * (1.0 - hn * 0.5);

    // Temperature: white-hot at the base, orange mid, deep red at the cooling tips.
    float temp = core * (1.0 - smoothstep(0.0, 1.0, hn) * 0.62);
    ...
    col        = mix(col, vec3(1.0, 0.94, 0.72),  smoothstep(0.78, 1.0,  temp));

...
- **window** draws over each window in place, regardless of focus, including
  ...
  with client-side decorations and `corner_radius = 0` has no corner box for
  the mask to follow, so the effect also shades its client-side margin band.
...
- **cursor** draws in a square of `radius` logical pixels around the pointer
  ...
  currently holding the pointer, clipped at that output's edges. It hides
  when the compositor hides the pointer; a client that hides its own cursor
  image does not by itself turn the effect off. In a screen- or cursor-only
  setup, a capture with `in_capture = false` composes the frame twice: once
  for the effect and once, unfiltered, for the capture.
...
Border light is built from the ring in buffer pixels, so the same preset
emits brighter on a higher-scale output than on scale 1.
